...
static ssize_t waitq_read(struct file *filp, char __user *buf, size_t len,loff_t * off);
static ssize_t waitq_write(struct file *filp, const char *buf, size_t len, loff_t * off);
...
*/
static ssize_t waitq_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
...
static int __init waitq_driver_init(void)
{
        ...
        if((alloc_chrdev_region(&dev, 0, 1, "waitq_Dev")) <0){
        ...
        if((cdev_add(&waitq_cdev,dev,1)) < 0){
            pr_err("Cannot add the device to the system\n");
            goto r_class;
        }
 
        /*Creating struct class*/
        if((dev_class = class_create(THIS_MODULE,"waitq_class")) == NULL){
            pr_err("Cannot create the struct class\n");
            goto r_class;
        }
 
        /*Creating device*/
        if((device_create(dev_class,NULL,dev,NULL,"waitq_device")) == NULL){
