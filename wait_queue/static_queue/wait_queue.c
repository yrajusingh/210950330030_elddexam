...
static int __init queue_driver_init(void);
static void __exit queue_driver_exit(void);
 
/*************** Driver Functions **********************/
static int queue_open(struct inode *inode, struct file *file);
static int queue_release(struct inode *inode, struct file *file);
static ssize_t queue_read(struct file *filp, char __user *buf, size_t len,loff_t * off);
static ssize_t queue_write(struct file *filp, const char *buf, size_t len, loff_t * off);
...
*/
static ssize_t queue_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
...
static int __init queue_driver_init(void)
{
        ...
        if((alloc_chrdev_region(&dev, 0, 1, "queue_Dev")) <0){
        ...
        queue_cdev.owner = THIS_MODULE;
        queue_cdev.ops = &fops;
 
        /*Adding character device to the system*/
        if((cdev_add(&queue_cdev,dev,1)) < 0){
            pr_err("Cannot add the device to the system\n");
            goto r_class;
        }
 
        /*Creating struct class*/
        if((dev_class = class_create(THIS_MODULE,"queue_class")) == NULL){
            pr_err("Cannot create the struct class\n");
            goto r_class;
        }
 
        /*Creating device*/
        if((device_create(dev_class,NULL,dev,NULL,"queue_device")) == NULL){
